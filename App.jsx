...
// --- CONFIGURATION GLOBALE (DÉFINIE HORS COMPOSANT POUR ÉVITER LES ERREURS) ---
  ...
  return (
    <div id="print-target" className="bg-white text-left p-0 m-0 font-sans">
          ...
          <div className="p-8 bg-blue-50/50 border border-blue-100 rounded-[2rem]">
            ...
            <div className="space-y-4">
              {agencyAudit.map((item, i) => {
                ...
                return (
                  <div key={`ag-print-${i}`} className="flex items-start gap-4">
      ...
      {dataSummary.collabs.map((c) => {
        ...
        return (
          <div key={`p-col-${normKey}`} className="print-page flex flex-col text-left mb-10 border-b border-slate-100">
            ...
            <div className="space-y-6 flex-1 text-left text-left">
              <div className="p-6 bg-slate-50/50 rounded-[1.5rem] border border-blue-50 text-left">
                ...
                <div className="space-y-4">
                  {audit.map((item, i) => {
                    const IconComp = item.icon;
                    return (
                      <div key={`print-diag-${normKey}-${i}`} className="flex items-start gap-5">
                        <div className={`mt-1.5 p-2 rounded-lg shrink-0 ${item.met ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'}`}>
                          {IconComp && <IconComp size={24} />}
                        </div>
              ...
              <div className="p-6 bg-emerald-50/50 rounded-[1.5rem] border border-emerald-100 text-left text-left">
                ...
                <p className="text-xs font-bold text-emerald-950 italic leading-relaxed whitespace-pre-wrap">{managerComments[normKey] || "Maintenez la rigueur."}</p>
              </div>
            </div>
          </div>
        );
    ...
    <div className="flex h-screen bg-white text-slate-900 overflow-hidden font-sans text-left text-sm text-left">
      <aside className="w-64 bg-[#0033a0] text-white p-6 flex flex-col gap-8 print:hidden shrink-0 relative z-20 shadow-2xl text-left">
        <div className="flex items-center gap-3">
          ...
          <div className="text-left text-left"><span className="font-black tracking-tighter uppercase text-sm block leading-none">EM Executive</span><span className="text-[7px] text-blue-200 font-bold tracking-[0.2em] uppercase text-left text-left">v60.9 Ultra Stable</span></div>
        </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative bg-[#F4F7FF] print:bg-white text-left">
        <header className="h-16 bg-white border-b border-blue-100 px-8 flex items-center justify-between shrink-0 print:hidden z-10 text-left text-left">
          <div className="flex items-center gap-4 text-left">
            <h2 className="font-black uppercase tracking-tight italic text-sm text-[#0033a0] text-left text-left">Dashboard du {today}</h2>
            <div className="h-6 w-px bg-slate-100 hidden md:block text-left text-left text-left text-left text-left"></div>
            <span className="px-3 py-1 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-lg text-[9px] font-black uppercase italic tracking-widest text-left text-left">Ultra v60.9</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 print:p-0">
              ...
              <div className={CARD_CLASS}>
                ...
                {dataSummary.count > 0 && (
                  <div className="flex items-center gap-4 p-4 bg-blue-50 border border-blue-100 rounded-2xl mb-8 animate-in zoom-in duration-300">
                    ...
                    <div className="text-left text-left text-left text-left text-left text-left"><p className="text-[10px] font-black text-[#0033a0] uppercase tracking-widest leading-none text-left text-left text-left text-left">Analyse nominative</p><p className="text-sm font-bold text-slate-700 mt-1">Le système a identifié <span className="text-[#0033a0] font-black">{dataSummary.count}</span> collaborateur(s).</p></div>
                  </div>
                )}
            ...
            <div className="max-w-4xl mx-auto space-y-8 pb-20 animate-in fade-in duration-700 text-left text-left text-left text-left text-left">
              <div className="bg-[#0033a0] rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden text-left text-left text-left text-left text-left text-left">
                <div className="flex items-center gap-4 mb-8 text-left text-left text-left text-left text-left text-left">
                  ...
                  <div className="text-left text-left text-left text-left"><h3 className="text-2xl font-black uppercase tracking-tighter leading-none italic text-white text-left text-left text-left text-left text-left">Bilan Agence Global</h3><p className="text-[9px] font-bold text-blue-200 uppercase tracking-[0.2em] mt-2 text-left text-left text-left">Portrait de Performance ({dataSummary.range})</p></div>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-6 gap-3 mb-8 text-left text-left text-left text-left text-left text-left text-left">
                ...
                <div className="grid grid-cols-1 gap-4 text-left">
                  <div className="bg-white/5 border border-white/10 rounded-3xl p-6 text-left text-left text-left text-left text-left text-left">
                    <div className="flex items-center gap-2 mb-4 text-blue-100 font-black text-[10px] uppercase tracking-widest text-left text-left text-left text-left text-left text-left"><Activity size={14}/> Diagnostic Automatique</div>
                    <div className="space-y-4 text-left text-left text-left">
                      {agencyAudit.map((item, i) => {
                        const IconComponent = item.icon;
                        return (
                          <div key={`ag-audit-${i}`} className={`p-4 rounded-2xl border flex items-start gap-4 transition-all ${item.met ? 'bg-white/10 border-white/20' : 'bg-rose-500/20 border-rose-500/30'}`}>
                             <div className={`mt-1.5 p-2 rounded-lg shrink-0 ${item.met ? 'bg-emerald-400 text-emerald-900' : 'bg-rose-400 text-rose-900'}`}>
                                {IconComponent && <IconComponent size={28} />}
                             </div>
                             <div className="text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left"><span className="text-[8px] font-black uppercase opacity-60 block text-left text-left text-left text-left">{item.label}</span><p className="text-xs font-bold leading-snug">{item.summary}</p><p className="text-[10px] opacity-70 mt-1 italic text-left text-left text-left text-left text-left text-left text-left">{item.numericalDetail}</p></div>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-3xl p-6 text-left text-left text-left text-left text-left">
                     ...
                     <div className="p-4 bg-white/5 rounded-2xl border border-white/10 min-h-[200px] text-left">
                        {agencyComment ? (
                           <p className="text-sm font-bold text-white italic leading-relaxed whitespace-pre-wrap text-left text-left text-left text-left text-left text-left text-left">{agencyComment}</p>
                        ) : (
              ...
              {dataSummary.collabs.map((c) => (
                <div key={`card-${c.name}`} className={CARD_CLASS}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left text-left">
                    <div className="p-6 bg-blue-50/40 border border-blue-100 rounded-3xl shadow-inner text-left text-left text-left text-left text-left text-left text-left">
                        ...
                        {(analysisResults[c.name] || []).map((item, i) => {
                          ...
                          return (
                            <div key={`diag-res-${c.name}-${i}`} className={`p-4 rounded-2xl border flex items-start gap-4 transition-all ${item.met ? 'bg-white border-emerald-100' : 'bg-rose-50/50 border-rose-100'}`}>
                              ...
                              <div className="text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left"><span className="text-[8px] font-black uppercase opacity-40 block text-left text-left text-left text-left">{item.label}</span><p className="text-xs font-black leading-snug">{item.summary}</p><p className="text-[10px] font-bold text-slate-500 mt-1 italic leading-relaxed text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">{item.numericalDetail}</p></div>
                            </div>
                          );
                        })}
                      </div>
                    </div>
                    <div className="p-6 bg-emerald-50/40 border border-emerald-100 rounded-3xl shadow-inner text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
                      <div className="flex items-center gap-2 mb-3 text-emerald-700 font-black text-[10px] uppercase tracking-widest text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left"><ThumbsUp size={14}/> Directives Manager</div>
                      <div className="p-4 bg-white/50 rounded-2xl border border-slate-100 min-h-[150px] text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
                         <p className="text-sm font-bold text-emerald-900 italic leading-relaxed whitespace-pre-wrap text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">{managerComments[c.name.toLowerCase().replace(/\s/g, '')] || "Saisissez vos conseils dans l'onglet coaching."}</p>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          {tab === 'config' && (
            <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
              <div className={CARD_CLASS}>
                 <div className="flex items-center gap-4 mb-8 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
              ...
              <div className="space-y-4 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
                 <div className="flex items-center gap-4 mb-6 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
                    <div className="p-3 bg-blue-50 text-[#0033a0] rounded-2xl text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left"><UserCog size={24}/></div>
                    <h3 className="text-xl font-black uppercase tracking-tighter text-slate-900 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">Commentaires par Collaborateur</h3>
                 </div>
                 {[...new Set(rawEntries.map(e => e.name))].map(name => {
                    const key = name.toLowerCase().replace(/\s/g, '');
                    return (
                      <div key={`input-row-${key}`} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col md:flex-row gap-6 items-start text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
                         <div className="flex items-center gap-4 min-w-[200px] text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
                            <div className="w-10 h-10 rounded-xl bg-indigo-50 text-indigo-700 flex items-center justify-center font-black text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">{name[0]}</div>
                            <span className="font-black uppercase text-slate-900 tracking-tight text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">{name}</span>
                         </div>
      ...
      {showPdf && (
        <div className="fixed inset-0 z-[100] bg-blue-900/95 backdrop-blur-xl flex flex-col p-4 animate-in fade-in duration-300 overflow-hidden text-left print:hidden text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
          <div className="flex justify-between text-white mb-4 px-4 max-w-5xl mx-auto w-full text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
            <div className="flex items-center gap-3 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
              ...
              <span className="font-black uppercase tracking-widest italic text-xs text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">Aperçu Portrait A4 Standard</span>
            </div>
            <div className="flex items-center gap-4 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
               <button onClick={handlePrint} className="px-6 py-3 bg-white text-[#0033a0] font-black rounded-xl flex items-center gap-2 shadow-2xl text-[10px] uppercase hover:bg-blue-50 transition-all text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">Impression Système</button>
          ...
          <div className="flex-1 overflow-auto bg-slate-200/20 p-4 flex flex-col items-center text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
            <div className="bg-white shadow-2xl p-0 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left" style={{ width: A4_WIDTH }}>
