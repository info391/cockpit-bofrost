...
const CollaboratorAuditSection = ({ name, data, analysisItems, badges, actionPlan }) => {
  ...
  return (
    <div className="mb-4 bg-white p-6 rounded-[2rem] border border-slate-100 shadow-md overflow-hidden relative page-collaborator print:mb-0 print:border-none print:shadow-none print:p-0 text-left">
      <div className="bg-slate-50/30 -mx-6 -mt-6 p-4 border-b border-slate-100 mb-4 print:bg-white print:p-1 print:mb-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-indigo-50/20 rounded-xl p-4 border border-indigo-100/30 relative text-left print:p-2 print:border-none print:bg-white">
        ...
        <div className="bg-emerald-50/30 rounded-xl p-4 border border-emerald-100/40 relative text-left print:p-2 print:border-none print:bg-white text-left">
          ...
          <div className="p-4 bg-white/5 rounded-lg border border-emerald-100 min-h-[100px] print:p-1 print:border-none">
            {actionPlan ? (
              ...
              <p className="text-slate-300 text-xs italic">Saisissez des objectifs pour cette période.</p>
            )}
          </div>
        </div>
      </div>
...
export default function App() {
  ...
  const handleAnalyse = async () => {
    ...
    // MÉTHODE ULTRA STABLE : Prompt fusionné (pas de champ systemInstruction)
          ...
          {activeTab === 'import' && (
            <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 text-left">
              <div className="bg-white rounded-[2rem] p-10 shadow-xl border border-slate-100 relative overflow-hidden text-left">
                ...
                <div className="mt-8 flex justify-end text-left text-left">
               ...
               <div className="bg-white rounded-[2rem] p-10 shadow-xl border border-slate-100 text-left">
                  ...
                  <div className="space-y-6 text-left">
                     <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl text-amber-800 text-[12px] font-bold shadow-sm text-left text-left text-left">
                        <p className="mb-2 uppercase tracking-wider italic text-left">Aide à la connexion :</p>
                        ...
                        <p className="mt-1 font-normal opacity-80 italic text-left text-left text-left">Note : Utilisez bien une clé "Google AI Studio" (commençant par AIza) et non une clé "Google Cloud Platform".</p>
                     </div>
      ...
      {showApercu && (
        <div className="fixed inset-0 z-[100] bg-slate-950/95 backdrop-blur-md flex flex-col items-center p-4 overflow-hidden text-left text-left">
           <div className="w-full max-w-7xl flex items-center justify-between mb-3 text-white px-2 text-left">
              <div className="flex items-center gap-3 text-left text-left"><div className="p-2 bg-indigo-600 rounded-lg text-left"><Eye size={18}/></div><div><h3 className="text-lg font-black uppercase tracking-widest leading-none italic tracking-tighter text-left">Rapport Prêt pour Diffusion</h3></div></div>
              <div className="flex items-center gap-4 text-left text-left"><button onClick={exportToPDF} disabled={isExporting} className="px-8 py-3.5 bg-emerald-500 hover:bg-emerald-600 text-white font-black rounded-xl flex items-center gap-3 shadow-2xl text-base uppercase disabled:opacity-50 tracking-tighter cursor-pointer text-left">{isExporting ? <Loader2 className="animate-spin" size={18}/> : <FileDown size={22}/>} {isExporting ? "Calcul..." : "Télécharger PDF"}</button><button onClick={() => setShowApercu(false)} className="p-2 bg-white/10 hover:bg-white/20 rounded-full transition-all cursor-pointer text-left"><X size={24}/></button></div>
           </div>
           <div className="flex-1 w-full bg-slate-800 rounded-2xl overflow-y-auto p-6 shadow-inner text-left text-left text-left text-left">
              <div className="bg-white mx-auto shadow-2xl print-wrapper text-left text-left text-left" style={{ width: '280mm' }} id="print-area"><div className="p-10 text-left text-left text-left text-left"><div className="flex items-center gap-4 mb-4 pb-4 border-b border-slate-100 text-left text-left text-left text-left text-left text-left text-left"><ShieldCheck size={32} className="text-indigo-600"/><div className="flex flex-col text-left text-left text-left text-left text-left text-left"><h1 className="text-2xl font-black text-slate-900 uppercase tracking-tighter leading-none italic text-left text-left text-left text-left text-left">Audit Stratégique Hebdomadaire - {todayDate}</h1><p className="text-[9px] text-slate-400 font-black uppercase tracking-[0.4em] mt-1 italic tracking-widest text-left text-left text-left text-left text-left text-left">Dossiers de Performance EMconsulting ({periodText})</p></div></div>{auditContent}</div></div>
           </div>
        </div>
      )}
