...
export default function App() {
  ...
  const handleAnalyse = async () => {
    ...
    // MÉTHODE ZÉRO ÉCHEC : On fusionne les instructions système avec les données
    const instructions = `Tu es l'Expert Coach EMconsulting Bofrost spécialisé dans l'analyse de performance de vente à domicile.
    
    TA STRUCTURE DE RÉPONSE DOIT ÊTRE RIGOUREUSE :
    ...
    // Stratégie de modèles : Priorité aux modèles Flash qui sont les plus tolérants sur le quota
    const attempts = [
    ...
    for (const config of attempts) {
      ...
      try {
        ...
        // Payload simplifié à l'extrême (sans champ system_instruction qui cause des erreurs 400)
        const body = { 
    ...
    if (!success) {
      setErrorMsg(`Échec de l'Analyse. Détails des tentatives :\n${errors.join('\n')}\n\nNote : Si l'erreur persiste, assurez-vous d'utiliser une clé "Google AI Studio" générée sur aistudio.google.com.`);
    }
  ...
  const auditContent = useMemo(() => {
    ...
    if (!analysis.includes('[SECTION_START]') && !analysis.includes('[COLLAB_START]')) {
      return <div className="max-w-4xl mx-auto bg-white p-10 rounded-[2rem] shadow-xl text-left whitespace-pre-wrap text-slate-700 leading-relaxed font-medium text-left">{analysis}</div>;
    }
    ...
    return items.map((item, idx) => {
      if (item.type === 'title') {
        return (
          <div key={idx} className="px-4 agency-summary-section text-left text-left">
    ...
    <div className="flex h-screen bg-[#F8FAFC] font-sans text-slate-900 overflow-hidden text-sm">
      <aside className="w-64 bg-indigo-950 text-white flex flex-col shadow-2xl z-20 print:hidden text-left text-left text-left">
        <div className="p-5 border-b border-white/10 bg-indigo-900/40">
          ...
          <p className="text-indigo-300 text-[7px] font-black uppercase tracking-[0.2em] opacity-60 italic text-left">Stable Release v18.4</p>
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar print:p-0 text-left text-left text-left">
          {activeTab === 'import' && (
            <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 text-left text-left text-left text-left text-left">
              <div className="bg-white rounded-[2rem] p-10 shadow-xl border border-slate-100 relative overflow-hidden text-left text-left">
                ...
                <div className="mt-6 grid grid-cols-2 gap-4 text-left text-left text-left text-left">
                   <div className={`p-4 rounded-xl border flex items-center gap-3 ${collaborators.length > 0 ? 'bg-emerald-50 border-emerald-100 text-emerald-700' : 'bg-slate-50 border-slate-200 text-slate-400'}`}>
                      <Database size={20}/>
                      <div><p className="text-[10px] font-black uppercase tracking-tighter text-left text-left text-left text-left">Collaborateurs détectés</p><p className="text-lg font-bold text-left text-left text-left text-left">{collaborators.length}</p></div>
                   </div>
                   <div className={`p-4 rounded-xl border flex items-center gap-3 ${userApiKey ? 'bg-indigo-50 border-indigo-100 text-indigo-700' : 'bg-rose-50 border-rose-100 text-rose-700'}`}>
                      <Settings size={20}/>
                      <div><p className="text-[10px] font-black uppercase tracking-tighter text-left text-left text-left text-left text-left">Clé IA Gemini</p><p className="text-lg font-bold text-left text-left text-left text-left text-left">{userApiKey ? 'CONFIGURÉE' : 'MANQUANTE'}</p></div>
                   </div>
                </div>

                {errorMsg && <div className="mt-6 p-4 bg-rose-50 border border-rose-100 rounded-xl text-rose-800 text-[10px] font-mono whitespace-pre-wrap flex items-start gap-3 shadow-sm text-left text-left text-left text-left text-left text-left"><AlertCircle size={20} className="shrink-0"/> <span>{errorMsg}</span></div>}
                
                <div className="mt-8 flex justify-end text-left text-left text-left text-left">
                  <button onClick={handleAnalyse} disabled={loading || !pastedData} className="group px-10 py-4 bg-indigo-600 text-white rounded-xl font-black text-base shadow-2xl hover:bg-indigo-700 transition-all uppercase flex items-center gap-3 active:scale-95 cursor-pointer text-left text-left text-left text-left">
                    {loading ? (
                      <>
          ...
          {activeTab === 'analyse' && <div className="max-w-6xl mx-auto animate-in fade-in slide-in-from-right-12 duration-700 pb-24 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">{auditContent}</div>}
          {activeTab === 'plans' && (
            <div className="max-w-5xl mx-auto space-y-10 pb-24 text-left text-left text-left text-left text-left text-left text-left text-left text-left">
               {collaborators.map(name => (
                  <div key={name} className="bg-white rounded-[3rem] p-10 shadow-xl border border-slate-200 text-left text-left text-left text-left">
                     <div className="flex items-center gap-6 mb-8 text-left text-left text-left text-left text-left text-left"><div className="w-12 h-12 rounded-xl bg-indigo-50 text-indigo-700 flex items-center justify-center text-2xl font-black italic shadow-inner text-left text-left text-left text-left">{name.charAt(0)}</div><span className="text-3xl font-black text-slate-950 tracking-tighter uppercase text-left text-left text-left text-left">{name}</span></div>
                     <textarea className="w-full p-8 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-indigo-600 outline-none text-lg font-bold text-slate-700 leading-relaxed italic shadow-inner text-left text-left text-left text-left text-left" placeholder={`Directives de coaching pour ${name}...`} value={actionPlans[name.toLowerCase().replace(/\s/g, '')] || ''} onChange={(e) => setActionPlans({...actionPlans, [name.toLowerCase().replace(/\s/g, '')]: e.target.value})}/>
                  </div>
               ))}
               <button onClick={() => setActiveTab('analyse')} className="w-full py-8 bg-indigo-600 text-white rounded-[3rem] font-black text-2xl shadow-2xl hover:bg-indigo-700 uppercase tracking-tighter transform hover:-translate-y-1 transition-all text-center text-center text-center text-center text-center">Mettre à jour le rapport PDF</button>
            </div>
          )}
          {activeTab === 'config' && (
            <div className="max-w-2xl mx-auto text-left text-left text-left text-left text-left text-left text-left text-left text-left">
               <div className="bg-white rounded-[2rem] p-10 shadow-xl border border-slate-100 text-left text-left text-left text-left">
                  <h3 className="text-2xl font-black uppercase mb-6 flex items-center gap-3 text-indigo-600 italic tracking-tighter text-left text-left text-left text-left text-left"><Settings size={24}/> Paramètres IA</h3>
                  <div className="space-y-6 text-left text-left text-left text-left">
                     <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl text-amber-800 text-[12px] font-bold shadow-sm text-left text-left text-left text-left text-left">
                        <p className="mb-2 uppercase tracking-wider italic text-left text-left text-left text-left text-left">Aide à la connexion :</p>
                        <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-indigo-600 hover:underline text-left text-left text-left text-left text-left">
                           1. Créer une clé gratuite sur Google AI Studio <ExternalLink size={14}/>
                        </a>
                        <p className="mt-1 font-normal opacity-80 italic text-left text-left text-left text-left text-left text-left text-left">Note : Utilisez bien une clé "Google AI Studio" et non une clé "Google Cloud".</p>
                     </div>
                     <div className="text-left text-left text-left text-left text-left">
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 italic tracking-widest text-left text-left text-left text-left text-left text-left">Clé API Google Gemini (Format AIza...)</label>
                        <input type="password" value={userApiKey} onChange={(e) => saveApiKey(e.target.value)} className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-xl focus:border-indigo-600 outline-none font-mono text-sm shadow-inner text-left text-left text-left text-left text-left" placeholder="Collez votre clé ici..."/>
                        <p className="mt-2 text-[10px] text-slate-400 italic text-left text-left text-left text-left text-left text-left text-left">Cette clé est stockée uniquement dans votre navigateur.</p>
                     </div>
                  </div>
               </div>
            </div>
      ...
      {showApercu && (
        <div className="fixed inset-0 z-[100] bg-slate-950/95 backdrop-blur-md flex flex-col items-center p-4 overflow-hidden text-left text-left text-left text-left text-left text-left">
           <div className="w-full max-w-7xl flex items-center justify-between mb-3 text-white px-2 text-left text-left text-left text-left text-left">
              <div className="flex items-center gap-3 text-left text-left text-left text-left text-left text-left text-left"><div className="p-2 bg-indigo-600 rounded-lg text-left text-left"><Eye size={18}/></div><div><h3 className="text-lg font-black uppercase tracking-widest leading-none italic tracking-tighter text-left text-left text-left text-left text-left text-left text-left text-left">Rapport Prêt pour Diffusion</h3></div></div>
              <div className="flex items-center gap-4 text-left text-left text-left text-left text-left text-left text-left text-left"><button onClick={exportToPDF} disabled={isExporting} className="px-8 py-3.5 bg-emerald-500 hover:bg-emerald-600 text-white font-black rounded-xl flex items-center gap-3 shadow-2xl text-base uppercase disabled:opacity-50 tracking-tighter cursor-pointer text-left text-left text-left text-left text-left">{isExporting ? <Loader2 className="animate-spin" size={18}/> : <FileDown size={22}/>} {isExporting ? "Calcul..." : "Télécharger PDF"}</button><button onClick={() => setShowApercu(false)} className="p-2 bg-white/10 hover:bg-white/20 rounded-full transition-all cursor-pointer text-left text-left text-left text-left"><X size={24}/></button></div>
           </div>
           <div className="flex-1 w-full bg-slate-800 rounded-2xl overflow-y-auto p-6 shadow-inner text-left text-left text-left text-left text-left text-left text-left">
              <div className="bg-white mx-auto shadow-2xl print-wrapper text-left text-left text-left" style={{ width: '280mm' }} id="print-area"><div className="p-10 text-left text-left text-left text-left text-left text-left text-left text-left text-left"><div className="flex items-center gap-4 mb-4 pb-4 border-b border-slate-100 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left"><ShieldCheck size={32} className="text-indigo-600"/><div className="flex flex-col text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left"><h1 className="text-2xl font-black text-slate-900 uppercase tracking-tighter leading-none italic text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">Audit Stratégique Hebdomadaire - {todayDate}</h1><p className="text-[9px] text-slate-400 font-black uppercase tracking-[0.4em] mt-1 italic tracking-widest text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">Dossiers de Performance EMconsulting ({periodText})</p></div></div>{auditContent}</div></div>
           </div>
        </div>
      )}
