...
const runDetailedAudit = (averages) => {
  ...
  return [
    check(
      ...
      "L'accroche est excellente.", "L'accroche est à retravailler.",
      "Votre excellence dans l'accroche initiale est un levier majeur de votre performance. En maîtrisant parfaitement les premières secondes de l'échange, vous parvenez à briser la barrière de méfiance naturelle des foyers, ce qui vous permet d'augmenter significativement le nombre de présentations complètes. Ce ratio bas démontre une communication non-verbale rassurante et un discours percutant qui valorise immédiatement l'intérêt de vous écouter. Vous évitez ainsi l'épuisement inutile et maximisez chaque opportunité de contact.",
      "Votre ratio indique une fragilité importante dans votre phase d'approche. Actuellement, vous dépensez une énergie considérable pour un nombre d'écoutes trop restreint, ce qui nuit à votre moral et à votre rentabilité. Il est impératif de retravailler vos trente premières secondes : votre ton, votre posture et votre phrase d'accroche doivent être plus percutants pour susciter une curiosité immédiate. Un ratio trop élevé ici est souvent le signe d'un manque de conviction initiale qui pousse les clients à couper court à l'échange.",
      true
    ),
    check(
      averages.rPresProsp, 2, "Présents / Prospects", 
      "Découverte client maîtrisée.", "Phase de découverte insuffisante.",
      "Votre capacité à transformer un présent en prospect montre que vous savez identifier avec précision les besoins réels du foyer durant votre présentation. En posant les bonnes questions ouvertes, vous amenez le client à verbaliser ses attentes, ce qui facilite grandement la suite du processus. Vous ne faites pas qu'exposer des produits, vous apportez des solutions concretes, ce qui crée une valeur perçue élevée. Continuez à soigner cette étape de qualification qui est le socle de votre réussite actuelle.",
...
export default function App() {
  ...
  const [agencyComment, setAgencyComment] = useState(''); // Nouveau state pour l'agence
  ...
  const dataSummary = useMemo(() => {
    ...
    const range = `${selectedMonths.length > 0 ? selectedMonths.join(', ') : 'Tout'} | ${selectedWeeks.length > 0 ? selectedWeeks.join(', ') : 'Toutes semaines'}`;
  ...
  const handlePrint = () => {
    ...
    win.document.write(`<html><head>${styles}<style>
      ...
      .print-page { width: 210mm; height: 296mm; padding: 15mm; box-sizing: border-box; page-break-after: always; display: flex; flex-direction: column; text-align: left; }
    ...
    <div className="flex h-screen bg-white text-slate-900 overflow-hidden font-sans text-left text-sm">
      <aside className="w-64 bg-[#0033a0] text-white p-6 flex flex-col gap-8 print:hidden shrink-0 relative z-20 shadow-2xl text-left">
        <div className="flex items-center gap-3">
          ...
          <div><span className="font-black tracking-tighter uppercase text-sm block leading-none">EM Executive</span><span className="text-[7px] text-blue-200 font-bold tracking-[0.2em] uppercase">v49.0 Manager</span></div>
        </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative bg-[#F4F7FF] print:bg-white text-left">
        <header className="h-16 bg-white border-b border-blue-100 px-8 flex items-center justify-between shrink-0 print:hidden z-10 text-left">
          <div className="flex items-center gap-4 text-left">
            ...
            <span className="px-3 py-1 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-lg text-[9px] font-black uppercase italic tracking-widest leading-none">Management v49</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 print:p-0">
              ...
              <div className={cardClass}>
                ...
                <div className="mt-8 flex items-center justify-between pt-6 border-t border-slate-100 text-left">
                  <div className="flex gap-10">
                    ...
                    <div className="flex flex-col"><span className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-1 text-left text-left text-left">Statut</span><span className="text-sm font-black text-[#0033a0] leading-none uppercase italic">Prêt</span></div>
                  </div>
            ...
            <div className="max-w-6xl mx-auto space-y-8 pb-20 animate-in fade-in duration-700 text-left text-left text-left">
              ...
              <div className="bg-[#0033a0] rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden text-left">
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <div><h3 className="text-2xl font-black uppercase tracking-tighter leading-none italic text-white text-left">Bilan Agence Global</h3><p className="text-[9px] font-bold text-blue-200 uppercase tracking-[0.2em] mt-2 text-left">Performance consolidée de la structure</p></div>
                </div>
                  ...
                  <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-3xl p-6">
                     ...
                     <div className="p-4 bg-white/5 rounded-2xl border border-white/10 min-h-[200px]">
                        {agencyComment ? (
                           ...
                           <p className="text-blue-200/50 text-xs italic text-left">Aucune directive globale saisie pour l'agence.</p>
                        )}
                     </div>
                  </div>
                </div>
                ...
                <div key={`collab-card-${c.name}`} className={cardClass}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left text-left text-left">
                    ...
                    <div className="p-6 bg-emerald-50/40 border border-emerald-100 rounded-3xl shadow-inner text-left text-left text-left text-left">
                      ...
                      <textarea className="w-full h-full min-h-[300px] bg-transparent border-none outline-none text-emerald-950 font-bold placeholder:text-emerald-300 resize-none italic text-base text-left text-left text-left text-left" value={managerComments[c.name] || ''} onChange={(e) => setManagerComments({...managerComments, [c.name]: e.target.value})} placeholder="Saisissez ici vos conseils personnalisés..."/>
                    </div>
                  </div>
                </div>
              ))}
          ...
          {tab === 'config' && (
            <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500 text-left">
              {/* SECTION DIRECTIVES AGENCE */}
              <div className={cardClass}>
                 <div className="flex items-center gap-4 mb-8">
                    <div className="p-3 bg-indigo-50 text-[#0033a0] rounded-2xl"><Building2 size={24}/></div>
              ...
              <div className="p-6 bg-slate-50/50 rounded-3xl border border-blue-50 text-center text-slate-400 font-black uppercase text-[10px] text-left text-left text-left">
                 Les directives saisies ici s'afficheront sur la première page du dossier PDF.
              </div>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-[100] bg-blue-900/95 backdrop-blur-xl flex flex-col p-4 animate-in fade-in duration-300 overflow-hidden text-left print:hidden">
          <div className="flex justify-between text-white mb-4 px-4 max-w-7xl mx-auto w-full text-left">
            <div className="flex items-center gap-3 text-left">
              ...
              <span className="font-black uppercase tracking-widest italic text-xs text-left">Dossier Manager v49</span>
            </div>
            <div className="flex items-center gap-4 text-left">
            ...
            <div className="bg-white shadow-2xl w-[210mm] p-0 text-left" id="print-area">
                ...
                <div className="print-page text-left text-left text-left">
                    <div className="flex items-center justify-between mb-8 pb-4 border-b-2 border-blue-50">
                       <div className="flex items-center gap-5">
                          ...
                          <div className="text-left text-left text-left">
                            ...
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1 leading-none text-left text-left">Executive Management Report • {today}</p>
                    ...
                    <div className="mb-8 text-left text-left text-left text-left">
                      <div className="text-[9px] font-black text-[#0033a0] uppercase tracking-widest mb-4 flex items-center gap-2 text-left"><Users size={14}/> Moyennes Globales de l'Équipe</div>
                    ...
                    <div className="space-y-6 flex-1 text-left text-left text-left">
                      <div className="p-8 bg-blue-50/50 border border-blue-100 rounded-[2rem] text-left text-left text-left">
                        <div className="text-[9px] font-black text-[#0033a0] uppercase mb-4 text-left text-left">Diagnostic Stratégique Collectif</div>
                        <div className="space-y-4">
                          {agencyAudit.map((item, i) => (
                      ...
                      <div className="p-8 bg-emerald-50 border border-emerald-100 rounded-[2rem] text-left text-left">
                        <div className="text-[9px] font-black text-emerald-600 uppercase mb-4 text-left text-left">Directives Stratégiques Agence</div>
                        <p className="text-sm font-bold text-emerald-950 italic leading-relaxed whitespace-pre-wrap text-left">
                          {agencyComment || "Poursuivez les efforts sur la prospection et maintenez la rigueur sur le terrain pour garantir les résultats du mois."}
                        </p>
                      </div>
                    </div>
                </div>
                  ...
                  return (
                    <div key={`pdf-collab-${c.name}`} className="print-page text-left text-left text-left text-left text-left">
                      <div className="flex items-center gap-4 mb-6 text-left text-left text-left text-left">
                         <div className="w-10 h-10 rounded-xl bg-[#0033a0] text-white flex items-center justify-center font-black text-lg text-left text-left">{c.name[0]}</div>
                         <h3 className="text-2xl font-black uppercase tracking-tighter text-slate-900 border-b-4 border-[#0033a0] pb-1 inline-block text-left text-left text-left">{c.name}</h3>
                      </div>
                      <div className="grid grid-cols-6 gap-2 mb-8 text-left text-left text-left text-left">
                      ...
                      <div className="space-y-6 flex-1 text-left text-left text-left text-left text-left">
                        <div className="p-6 bg-slate-50/50 rounded-[1.5rem] border border-blue-50 text-left text-left text-left text-left text-left">
                          <div className="text-[8px] font-black text-[#0033a0] uppercase mb-4 tracking-widest text-left text-left text-left text-left text-left text-left">Diagnostic Individuel Expert</div>
                             ...
                             {detailedAudit.map((item, i) => (
                               <div key={`pdf-diag-item-${c.name}-${i}`} className="flex items-start gap-4 text-left text-left text-left">
                                 ...
                                 <div className="flex flex-col text-left text-left text-left text-left">
                                   <span className="text-[8px] font-black uppercase opacity-40 leading-none text-left text-left text-left text-left text-left">{item.label}</span>
                                   <p className="text-[10px] font-black text-slate-800 leading-tight mt-0.5 text-left text-left text-left text-left text-left">{item.summary}</p>
                                   <p className="text-[8px] font-bold text-slate-500 mt-1 italic leading-relaxed text-left text-left text-left text-left text-left">{item.numericalDetail}</p>
                                 </div>
                               </div>
                             ))}
                          </div>
                        ...
                        <div className="p-6 bg-emerald-50/50 rounded-[1.5rem] border border-emerald-100 text-left text-left text-left text-left text-left">
                          <div className="text-[8px] font-black text-emerald-600 uppercase mb-3 tracking-widest text-left text-left text-left text-left text-left text-left">Commentaires Manager</div>
                          <p className="text-xs font-bold text-emerald-950 italic leading-relaxed whitespace-pre-wrap text-left text-left text-left text-left text-left text-left">
